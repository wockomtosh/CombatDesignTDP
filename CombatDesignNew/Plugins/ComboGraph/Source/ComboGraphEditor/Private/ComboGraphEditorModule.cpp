...
#else
	const FSoftClassPath AbilitySystemGlobalsClassName = GetDefault<UGameplayAbilitiesDeveloperSettings>()->AbilitySystemGlobalsClassName;
#endif	
...
{
	CG_EDITOR_LOG(Verbose, TEXT("ComboGraphEditorModule - AddAbilitySystemGlobalsClassName token handler"))
	UGameplayAbilitiesDeveloperSettings* AbilitiesDevSettings = GetMutableDefault<UGameplayAbilitiesDeveloperSettings>();
	if (!AbilitiesDevSettings)
	{
		return;
	}

	AbilitiesDevSettings->AbilitySystemGlobalsClassName = UComboGraphAbilitySystemGlobals::StaticClass();
	const FString DefaultConfigFilename = AbilitiesDevSettings->GetDefaultConfigFilename();
	CG_EDITOR_LOG(Verbose, TEXT("ComboGraphEditorModule - Should write to %s"), *DefaultConfigFilename)

	AbilitiesDevSettings->UpdateSinglePropertyInConfigFile(AbilitiesDevSettings->GetClass()->FindPropertyByName(GET_MEMBER_NAME_CHECKED(UGameplayAbilitiesDeveloperSettings, AbilitySystemGlobalsClassName)), DefaultConfigFilename);
