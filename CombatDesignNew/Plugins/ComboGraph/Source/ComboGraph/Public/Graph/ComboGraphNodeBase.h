...
private:
	/**
	 * Traverses the Edges map to lookup for edge matching Start Node (matching this node) and End Node (matching passed in
	 * Child Node).
	 *
	 * Acts as a fallback for cases where the map lookup and contains / find by hash is failing, which might happen with
	 * custom Combo Node implemented in BP and edited in editor while owning combo graph asset editor is closed.
	 *
	 * The next resave of Combo Graph asset should fixup the edges map but for the few occasions that happens, this fallback
	 * ensures we still are able to find the correct edge.
	 */
	UComboGraphEdge* GetEdgeComparingStartEndNodes(UComboGraphNodeBase* ChildNode);
};
