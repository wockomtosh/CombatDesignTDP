...
public:
	...
	/** In debug mode, traces will be rendered on clients too even if they are only setup for server hits registration */
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "Combo Graph|Debug", meta = (EditCondition = "bDebug", EditConditionHides))
	bool bDebugOnClient = false;
	...
	/** If set to true, hits will be registered on both server and clients */
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "Combo Graph|Traces")
	bool bRegisterHitsOnClients = false;
	...
	/** Delegate invoked when a successful hit is registered */
