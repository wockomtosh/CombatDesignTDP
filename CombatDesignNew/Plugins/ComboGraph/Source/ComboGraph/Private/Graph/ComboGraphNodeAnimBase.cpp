...
UWorld* UComboGraphNodeAnimBase::GetWorld() const
{
	return CachedOwningTask.IsValid() ? CachedOwningTask->GetWorld() : nullptr;
}
...
void UComboGraphNodeAnimBase::ApplyCueContainer(FComboGraphCueContainer* Container, const FGameplayEventData& EventData, bool bPassOnlySoftObjectPaths)
{
	...
	for (auto Definition : Definitions)
	{
		...
		if (ASC)
		{
			CG_RUNTIME_WLOG(Verbose, TEXT("Execute Gameplay Cue for %s"), *GameplayCueTag.ToString())
