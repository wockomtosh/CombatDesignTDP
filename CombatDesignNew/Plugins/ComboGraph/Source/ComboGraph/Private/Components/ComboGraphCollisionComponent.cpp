...
void UComboGraphCollisionComponent::DoTraceCheckForSocket(UPrimitiveComponent* InMesh, const FName InSocketName)
{
	...
	for (FHitResult HitResult : HitResults)
	{
		...
		{
			CG_RUNTIME_WLOG(Verbose, TEXT("Got a hit for Mesh %s but was ignored. Reason: %s"), *GetNameSafe(InMesh), *FailReason);
		}
	}
}
...
void UComboGraphCollisionComponent::HandleSuccessfulHit(const FHitResult InHitResult, AActor* HitActor, UPrimitiveComponent* HitComponent, UPrimitiveComponent* InMesh)
{
	if (bShouldLogHits)
	{
		CG_RUNTIME_WLOG(
			Display,
			TEXT("for Mesh: %s. Hit Actor: %s (Type: %s), Hit Component: %s (Profile: %s)"),
			*GetNameSafe(InMesh),
...
bool UComboGraphCollisionComponent::ShouldPerformTrace() const
{
	if (HasAuthority())
	{
		// We always want to do traces on server
		return true;
	}

	// At this point, we're on clients
	if (bDebugOnClient)
	{
		// Config is explicitly set to always perform traces on client, in order to debug them
		// even if it is marked as not successful hit later on in IsSuccessfulHit with bRegisterHitsOnClients set to false
		return true;
	}

	return bRegisterHitsOnClients;
}
...
bool UComboGraphCollisionComponent::IsSuccessfulHit(AActor* HitActor, UPrimitiveComponent* HitComponent, FString& FailReason) const
{
	...
	// Hits are set to not register on the client with RegisterHitsOnClients to false
	if (!bRegisterHitsOnClients && !HasAuthority())
	{
		FailReason = FString::Printf(TEXT("Hit Component %s with profile %s was ignored on this client because of RegisterHitsOnClients configuration."), *GetNameSafe(HitComponent), *HitComponent->GetCollisionProfileName().ToString());
		return false;
	}
